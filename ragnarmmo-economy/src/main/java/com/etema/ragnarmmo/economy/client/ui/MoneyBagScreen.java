package com.etema.ragnarmmo.economy.client.ui;
...
public class MoneyBagScreen extends Screen {
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float partialTicks) {
        ...
        g.drawString(this.font, Component.translatable("screen.ragnarmmo.money_bag.withdraw").withStyle(ChatFormatting.GRAY),
                MARGIN_X, 60, COLOR_MUTED, false);

        drawWithdrawButton(g, CARD_1_X, CARD_Y, CARD_W, CARD_H, ZenyItems.COPPER_ZENY.get(), coppers, mouseX, mouseY,
                "screen.ragnarmmo.money_bag.copper");
        drawWithdrawButton(g, CARD_2_X, CARD_Y, CARD_W, CARD_H, ZenyItems.SILVER_ZENY.get(), silvers, mouseX, mouseY,
                "screen.ragnarmmo.money_bag.silver");
        drawWithdrawButton(g, CARD_3_X, CARD_Y, CARD_W, CARD_H, ZenyItems.GOLD_ZENY.get(), golds, mouseX, mouseY,
                "screen.ragnarmmo.money_bag.gold");

        boolean canDeposit = invValue > 0;
        boolean hoverDep = canDeposit && isHovered(DEPOSIT_X, DEPOSIT_Y, DEPOSIT_W, DEPOSIT_H, mouseX, mouseY);

        int depFill = canDeposit ? (hoverDep ? 0x22384B2B : 0x161B2518) : 0x14000000;
        int depBorder = canDeposit ? (hoverDep ? COLOR_BORDER_HOVER : 0xFF47515C) : 0xFF3F3F3F;
        ...
        g.drawCenteredString(this.font, Component.translatable("screen.ragnarmmo.money_bag.deposit_all"),
                PANEL_WIDTH / 2, DEPOSIT_Y + 6, canDeposit ? COLOR_POSITIVE : COLOR_DISABLED);
    ...
    private void drawWithdrawButton(GuiGraphics g, int x, int y, int w, int h, Item item, long available,
            int mouseX, int mouseY, String labelKey) {
        boolean enabled = available > 0;
        boolean hovered = enabled && isHovered(x, y, w, h, mouseX, mouseY);

        int bg = enabled ? (hovered ? 0x22313F4D : 0x16161F27) : 0x12000000;
        int border = enabled ? (hovered ? COLOR_BORDER_HOVER : 0xFF586470) : 0xFF3F3F3F;
        ...
        g.drawCenteredString(this.font, Component.translatable(labelKey), x + w / 2, y + 27,
                enabled ? COLOR_MUTED : COLOR_DISABLED);
        g.drawCenteredString(this.font, Component.literal("x" + available), x + w / 2, y + 41,
