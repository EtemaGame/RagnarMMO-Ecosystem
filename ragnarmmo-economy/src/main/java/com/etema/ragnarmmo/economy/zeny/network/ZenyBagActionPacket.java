...
public class ZenyBagActionPacket {
    ...
    public static void handle(ZenyBagActionPacket msg, Supplier<NetworkEvent.Context> ctxSup) {
        ...
        ctx.enqueueWork(() -> {
            ...
            switch (msg.action) {
                case DEPOSIT_ALL -> handleDepositAll(player);
                case WITHDRAW_GOLD -> handleWithdraw(player, ZenyWalletHelper.GOLD_VALUE, ZenyItems.GOLD_ZENY.get());
                case WITHDRAW_SILVER -> handleWithdraw(player, ZenyWalletHelper.SILVER_VALUE, ZenyItems.SILVER_ZENY.get());
                case WITHDRAW_COPPER -> handleWithdraw(player, ZenyWalletHelper.COPPER_VALUE, ZenyItems.COPPER_ZENY.get());
            }
        });
    ...
    private static void handleWithdraw(ServerPlayer player, int value, net.minecraft.world.item.Item coinItem) {
        if (ZenyWalletHelper.tryConsume(player, value)) {
            ...
            player.playNotifySound(net.minecraft.sounds.SoundEvents.ITEM_PICKUP,
