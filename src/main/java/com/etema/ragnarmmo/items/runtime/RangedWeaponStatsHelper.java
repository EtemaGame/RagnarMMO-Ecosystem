...
public final class RangedWeaponStatsHelper {
    ...
    public static void snapshotArrow(AbstractArrow arrow, Player player, ItemStack weapon, float drawRatio) {
        ...
        snapshot.putInt("dex", totalStat(player, StatKeys.DEX));
        snapshot.putInt("luk", totalStat(player, StatKeys.LUK));
    ...
    private static int totalStat(Player player, StatKeys key) {
        if (!(player instanceof net.minecraft.server.level.ServerPlayer serverPlayer)) {
            return Math.max(1, (int) Math.round(StatAttributes.getTotal(player, key)));
        }
        return Math.max(1, (int) Math.round(StatAttributes.getTotal(player, key))
                + AcolyteSkillFormulaService.statusStatModifier(serverPlayer, key)
                + ArcherSkillFormulaService.statusStatModifier(serverPlayer, key));
    }
}
