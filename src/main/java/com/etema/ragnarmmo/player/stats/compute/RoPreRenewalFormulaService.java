...
public final class RoPreRenewalFormulaService {
    ...
    public static final int SHIELD_ASPD_PENALTY = 5;
    ...
    public static double criticalChance(int luk, double bonus) {
        return clamp(0.0D, 1.0D, ((1.0D + Math.max(0, luk) * 0.3D) / 100.0D) + bonus);
    }

    public static double perfectDodge(int luk) {
        return clamp(0.0D, 1.0D, (1.0D + Math.floor(Math.max(0, luk) / 10.0D)) / 100.0D);
    }
