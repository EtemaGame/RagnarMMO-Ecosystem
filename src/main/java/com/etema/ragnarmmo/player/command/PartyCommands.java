...
import com.etema.ragnarmmo.player.party.PartySettings;
 ...
 * /party settings loot <free|priority|roundrobin|off> - Set party loot mode
 * /party settings priority <seconds> - Set loot priority duration
 */
public class PartyCommands {
                ...
                .then(Commands.literal("settings")
                        ...
                        .then(Commands.literal("loot")
                                .then(Commands.literal("free")
                                        .executes(ctx -> setLootMode(ctx, PartySettings.LootMode.FREE)))
                                .then(Commands.literal("priority")
                                        .executes(ctx -> setLootMode(ctx, PartySettings.LootMode.PRIORITY)))
                                .then(Commands.literal("roundrobin")
                                        .executes(ctx -> setLootMode(ctx, PartySettings.LootMode.ROUND_ROBIN)))
                                .then(Commands.literal("off")
                                        .executes(ctx -> setLootMode(ctx, PartySettings.LootMode.OFF))))
                        .then(Commands.literal("priority")
                                .then(Commands.argument("seconds", IntegerArgumentType.integer(0, 120))
                                        .executes(ctx -> setLootPriority(ctx, IntegerArgumentType.getInteger(ctx, "seconds"))))));
    }
    ...
    private static int partyInfo(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        player.sendSystemMessage(Component.literal("\u00A77Loot Mode: \u00A7f" +
                party.getSettings().getLootMode().name().toLowerCase(java.util.Locale.ROOT)));
        player.sendSystemMessage(Component.literal("\u00A77Loot Priority: \u00A7f" +
                party.getSettings().getLootPrioritySeconds() + "s"));
    ...
    private static int setLootMode(CommandContext<CommandSourceStack> ctx, PartySettings.LootMode mode) throws CommandSyntaxException {
        ServerPlayer player = ctx.getSource().getPlayerOrException();
        PartyService service = PartyService.get(player.getServer());
        ...
        party.getSettings().setLootMode(mode);
        PartySavedData.get(player.getServer()).markDirty();
        service.syncPartyToMembers(party);

        player.sendSystemMessage(Component.literal("\u00A7aLoot mode set to " + mode.name().toLowerCase(java.util.Locale.ROOT) + "."));
        return 1;
    }

    private static int setLootPriority(CommandContext<CommandSourceStack> ctx, int seconds) throws CommandSyntaxException {
        ServerPlayer player = ctx.getSource().getPlayerOrException();
        PartyService service = PartyService.get(player.getServer());
        ...
        party.getSettings().setLootPrioritySeconds(seconds);
        PartySavedData.get(player.getServer()).markDirty();

        player.sendSystemMessage(Component.literal("\u00A7aLoot priority set to " + seconds + " seconds."));
        return 1;
    }
