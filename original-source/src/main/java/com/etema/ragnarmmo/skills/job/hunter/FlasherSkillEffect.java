...
public class FlasherSkillEffect implements ISkillEffect {

    private final HunterTrapManager.TrapDefinition definition;

    public FlasherSkillEffect() {
        this.definition = new HunterTrapManager.TrapDefinition(
                ...
                ParticleTypes.FLASH,
                (trap, target) -> {
                    // Trigger Logic: 15 seconds of Blindness + Glowing
                    target.addEffect(new MobEffectInstance(MobEffects.BLINDNESS, 20 * 15, 0, false, true));
                    target.addEffect(new MobEffectInstance(MobEffects.GLOWING, 20 * 15, 0, false, true));
                });
    }
