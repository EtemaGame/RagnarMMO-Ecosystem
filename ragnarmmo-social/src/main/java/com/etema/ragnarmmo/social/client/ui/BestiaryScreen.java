...
public class BestiaryScreen extends Screen {
    private static final String SOCIAL_MOD_ID = "ragnarmmo_social";
    ...
    @Override
    protected void init() {
        if (!ModList.get().isLoaded(SOCIAL_MOD_ID)) {
            return;
        }
        int x = panelX();
        int y = panelY();
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        if (!ModList.get().isLoaded(SOCIAL_MOD_ID)) {
            ...
            g.drawString(this.font, Component.translatable("gui.ragnarmmo.bestiary.title"), x + 10, y + 10, 0xFFE9D8A6, false);
            g.drawString(this.font, Component.translatable("gui.ragnarmmo.bestiary.unavailable"), x + 10, y + 30, 0xFFB6BECF, false);
        ...
        String version = "v" + BestiaryClientRegistry.version();
        g.drawString(this.font, version, x + PANEL_W - 10 - this.font.width(version), y + 9, 0xFF8F98A8, false);
